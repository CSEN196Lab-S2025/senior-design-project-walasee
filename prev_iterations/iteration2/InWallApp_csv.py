...
output_dir = 'walabotOut_csv'
makedirs(output_dir, exist_ok=True)

# Generate a unique filename with a timestamp
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
output_filename = join(output_dir, f'walabotOut_{timestamp}.csv')

# Writes walabot output to a csv file
def PrintSensorTargets(targets):
    with open(output_filename, 'a') as f:
        if targets:
            #print("Detected something...")
            for i, target in enumerate(targets):
                f.write('{},{},{},{}\n'.format(target.xPosCm, target.yPosCm, target.zPosCm, target.amplitude))
        else:
            print("No Target Detected")
            # f.write('No Target Detected\n')  # make so that there is still an output even when nothing is detected
