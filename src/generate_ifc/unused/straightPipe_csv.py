...
def read_coordinates_from_csv(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        for row in reader:
            if len(row) == 3:  # Ensure the row contains exactly 3 values
                x, y, z = map(float, row)
                coordinates.append([x, y, z])
    if len(coordinates) != 2:
        raise ValueError("The input CSV file must contain exactly two rows of coordinates (start and end points).")
    return np.array(coordinates[0]), np.array(coordinates[1])

# --------- Input: Read start and end Points from csv file
input_file = "coordinates.csv"  # Replace with your input csv file path
start, end = read_coordinates_from_csv(input_file)
